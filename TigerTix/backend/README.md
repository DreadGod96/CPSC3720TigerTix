# Helpful Resources for Sprint 1
- `https://medium.com/@jonoyanguren/mvc-pattern-in-nodejs-and-express-old-but-gold-46c21bee365a`
- `https://www.w3schools.com/nodejs/nodejs_rest_api.asp`
- `https://coderslegacy.com/javascript-sqlite3-tutorial-for-beginners-node-js/`
- `https://www.geeksforgeeks.org/node-js/express-js/`
- https://nodejs.org/en/learn/manipulating-files/reading-files-with-nodejs

# Helpful Resources for Sprint 2
- Task 1
    - https://medium.com/google-cloud/generating-request-body-for-apis-using-gemini-43977961ca2a
    - https://github.com/googleapis/js-genai
    - https://cookbook.openai.com/examples/how_to_use_guardrails
    - https://googleapis.github.io/js-genai/release_docs/index.html
- Task 2
    - https://developer.mozilla.org/en-US/docs/Web/API/Web_Speech_API/Using_the_Web_Speech_API
    - https://www.w3schools.in/javascript/text-to-speech-api
    - https://www.slingacademy.com/article/recognize-speech-with-the-web-speech-api-in-javascript/
- Task 3: Selenium
    - https://www.selenium.dev/documentation/
    - https://testingbot.com/resources/articles/react-selenium-testing
    - https://grapeup.com/blog/practical-tips-to-testing-react-apps-with-selenium/#
    - https://saucelabs.com/resources/blog/selenium-javascript-tutorial-web-automation

# Setting up Gemini
- Navigate to this link: https://aistudio.google.com/apikey
- Use a google account to create free api key, process should be simple
- Create .env file in backend/llm-service/.env
- Put this in the .env:
```txt
GEMINI_API_KEY=...
```
- Replace ...s with the api key


